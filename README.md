...
菜单中的“开机后自动启动”使用 macOS 公开的登录项接口，首次运行时默认勾选并尝试注册。启用后，MacMiddleClick 会在开机并进入桌面后自动运行。若系统需要用户批准，菜单会额外显示“需要在系统设置中允许自动启动…”，点击后打开“系统设置 → 通用 → 登录项”。用户主动取消勾选后，应用会记住该选择，不再尝试注册，也不会显示授权入口，直到用户重新勾选。该设置与中键映射的临时启用/禁用状态相互独立。
